...
final class ImageSubagentKind: SubagentKind, @unchecked Sendable {
    ...
    func resolveModel(_ scope: SubagentScope) async throws -> ResolvedModel {
        ...
        if !params.isEdit {
            let configuredTarget =
                isDefault ? config.defaultImageGenerationTarget : settings?.imageGenerationTarget
            let candidates = await MainActor.run {
                ModelPickerItemCache.shared.items.imageGenerationDelegateCandidates
            }
            let selected: ModelPickerItem?
            if let requested = params.model {
                let matches = candidates.filter {
                    $0.id == requested
                        || $0.mediaModel?.target.modelID == requested
                        || ($0.source.isImageGeneration && $0.id == requested)
                }
                selected = matches.count == 1 ? matches[0] : nil
            } else if let configuredTarget {
            ...
            guard let selected else {
                throw SubagentError.unavailable("No configured image generation model is available.")
            }
            let target =
    ...
    func permission(_ scope: SubagentScope, _ resolved: ResolvedModel) async -> SubagentDecision {
        ...
        if let minimum = selectedMediaModel?.pricing?.minimumUSD {
            approvalValues["estimated_minimum_usd"] = minimum
        }
        if selectedMediaModel != nil {
            approvalValues["billing_notice"] =
                "This image is generated by a billable remote provider."
        }
        ...
        if selectedMediaModel != nil {
            guard policy != .deny else {
                return .denied("Remote image generation is denied by this agent's permission settings.")
            }
            let approved = await ToolPermissionPromptService.requestApproval(
                toolName: "image",
                description:
                    "Generate a billable remote image. The provider, privacy policy, and estimated "
                    + "minimum price are shown in this approval.",
                argumentsJSON: approvalJSON
            )
            return approved
                ? .allow
                : .userDenied("User denied billable remote image generation.")
        }
    ...
    private static func backendDescription(_ model: MediaModelInfo) -> String {
    ...
    func run(
        ...
        if !params.isEdit, let target = resolvedTarget, target.backend != .local {
            do {
                let constraints = selectedMediaModel?.constraints ?? MediaModelConstraints()
                let usesPixelDimensions =
                    constraints.aspectRatios.isEmpty && constraints.resolutions.isEmpty
                let request = MediaImageGenerationRequest(
                    target: target,
                    prompt: params.prompt,
                    negativePrompt: params.negativePrompt,
                    width: usesPixelDimensions ? params.width : nil,
                    height: usesPixelDimensions ? params.height : nil,
                    ...
                    steps: params.steps,
                    guidance: params.guidance.map(Double.init),
                    seed: params.seed.flatMap { Int(exactly: $0) },
                    // Tool artifact enrichment currently displays one media
                    // artifact per call; never bill for hidden variants.
                    count: 1,
                    format: .png
                )
                ...
                var payload: [String: Any] = [
                    ...
                    "media_type": "image",
                    "mode": "generate",
                    "job_id": first.url.deletingPathExtension().lastPathComponent,
                    "model": first.modelID,
                    "status": "completed",
                    "already_displayed": true,
                    "display_note":
                        "The image is already shown to the user; do NOT call share_artifact for it.",
                    "images": generated.map {
                        [
                            "path": $0.url.path,
                ...
                if let settledCostUSD = first.settledCostUSD {
                    payload["settled_cost_usd"] = settledCostUSD
                }
                return SubagentResult(
                    payload: payload,
                    summary:
                        "Generated \(generated.count) image\(generated.count == 1 ? "" : "s") "
                        + "with \(resolved.name)."
                )
            } catch {
...
extension URL {
    static func osaurusImageFileURL(_ path: String) -> URL {
